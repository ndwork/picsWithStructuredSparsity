...
function [data,trueRecon] = loadSimData( simCase )
  ...
  switch simCase
    case 1
      ...
      sensitivities = simulateSliceSensitivities( img, 'H', 0.3 );  % H changed from 0.5
      %sensitivities = sensitivities / max( sensitivities(:) );

      nCoils = size( sensitivities, 3 );
      coilImgs = bsxfun( @times, sensitivities, img );
      fftCoilImgs = fftshift2( fft2( ifftshift2( coilImgs ) ) );
    ...
    case 2
      ...
      sensitivities = simulateSliceSensitivities( img, 'type', 'axial', 'H', 0.3 );
      %sensitivities = sensitivities / max( sensitivities(:) );
