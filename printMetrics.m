...
function printMetrics( logFile, datacase, sampleFraction, nSamples, recon, imgTitle, outDir, varargin )
  ...
  %     ' / ', num2str(sampleFraction), ' / ', num2str(vdSigFrac), ' / ', strjoin( optLogNames, ', ' ), ...
  %     ', ', logValuesString(1:end-1) ]);
  %   fprintf( logID, ['%d, %f, %d, ', imgTitle, ', %f, %d, %d, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f ', ...
  %     repmat( ', %d', [1 numel(optLogValues)] ), '\n' ], ...
  %     datacase, sampleFraction, nSamples, vdSigFrac, vdSig(1), vdSig(2), mse, mae, ssimValue, ...
  %     mdmScore, mdmScore2, niqeScore, piqeScore, ms_ssimValue, correlation, angleErr, ...
  %     mutualInfo, autofocusValue, optLogValues(:)' );
  % else
  %   disp([ 'Logging datacase ', num2str(datacase), ' / ', imgTitle, ...
  ...
  if numel( optLogValues ) > 0
    ...
    disp([ 'Logging datacase ', num2str(datacase), ' / ', imgTitle, ...
      ...
      ', ', logValuesString(1:end-1) ]);
    fprintf( logID, ['%d, %f, %d, ', imgTitle, ', %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f ', ...
      repmat( ', %d', [1 numel(optLogValues)] ), '\n' ], ...
      datacase, sampleFraction, nSamples, mse, mae, relErr, ssimValue, mdmScore, mdmScore2, niqeScore, ...
